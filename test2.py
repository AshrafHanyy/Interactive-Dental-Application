...
soc = socket.socket()
hostname = "localhost"  # Or use '127.0.0.1'
port = 65434
soc.bind((hostname, port))
soc.listen(1)  # Listen for a single connection
...
# Use live camera feed for prediction and send result through socket
def recognize_and_send_from_camera():
                ...
                for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                    ...
                    if frame_count == 50:
                        ...
                        if isinstance(result, list) and len(result) > 0 and hasattr(result[0], 'name'):
                            ...
                            # Send the recognized gesture over the socket connection
                            try:
                                conn.send(gesture_name.encode('utf-8'))
                            except Exception as e:
                                print("Error sending message:", e)
                        else:
                            print(f"Hand: {hand_label} | Recognized Gesture: None | Time taken: {end_time - start_time:.2f}s")
...
# Start the recognition and message-sending function
try:
    recognize_and_send_from_camera()
finally:
    conn.close()
    soc.close()
